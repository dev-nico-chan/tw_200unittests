...
namespace BasicCSharp
{
    public class Collections
    {
        ...
        {
            ISet<int> mySet = new HashSet<int>() { 1, 2, 3, 4, 5, 6, 7, 8 };

            // please update if statement fix the test.
            Action<int> elementValidator = (x) => { if (x > mySet.Count) throw new ArgumentOutOfRangeException("unexpected element"); };

            Assert.All(mySet, elementValidator);
            Assert.Equal(8, mySet.Count);
        }
